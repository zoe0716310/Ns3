 ...
 * disc. It has a single attribute, MyQueueDisc, used to set the child queue disc
 ...
 * as attributes.
 */
class MyQueueDiscClass : public Object
{
  public:
    /**
  ...
  private:
    Ptr<MyQueueDisc> m_queueDisc; //!< Queue disc attached to this class
};
 ...
 */
enum MyQueueDiscSizePolicy
{
    MY_SINGLE_INTERNAL_QUEUE,   //!< Used by queue discs with single internal queue
    MY_SINGLE_CHILD_QUEUE_DISC, //!< Used by queue discs with single child queue disc
    MY_MULTIPLE_QUEUES, //!< Used by queue discs with multiple internal queues/child queue discs
    MY_NO_LIMITS        //!< Used by queue discs with unlimited size
};
 ...
 */
class MyQueueDisc : public Object
{
  public:
     ...
     */
    MyQueueDisc(MyQueueDiscSizePolicy policy = MyQueueDiscSizePolicy::MY_SINGLE_INTERNAL_QUEUE);
     ...
     */
    MyQueueDisc(MyQueueDiscSizePolicy policy, QueueSizeUnit unit);

    ~MyQueueDisc() override;

    // Delete copy constructor and assignment operator to avoid misuse
    MyQueueDisc(const MyQueueDisc&) = delete;
    MyQueueDisc& operator=(const MyQueueDisc&) = delete;
     ...
     */
    void AddQueueDiscClass(Ptr<MyQueueDiscClass> qdClass);
     ...
     */
    Ptr<MyQueueDiscClass> GetQueueDiscClass(std::size_t i) const;
  ...
  protected:
     ...
     * \sa MyQueueDisc::InitializeParams
     * \sa MyQueueDisc::CheckConfig
     */
  ...
  private:
    ...
    std::vector<Ptr<MyQueueDiscClass>> m_classes; //!< Classes
    std::vector<Ptr<Ipv4QueueDiscItem>> ack_vector;
    std::vector<SequenceNumber32> acknum_vector;
    ...
    MyQueueDiscSizePolicy m_sizePolicy;    //!< The queue disc size policy
 ...
 */
std::ostream& operator<<(std::ostream& os, const MyQueueDisc::Stats& stats);

} // namespace ns3

#endif /* MY_QUEUE_DISC_H */
