...
uint32_t QueueLength = 30; // max packet number in aggregate table per worker
    ...
    if (Mycount1 % 1 == 0){
        ...
        if (copy->PeekHeader(tcpHeader) != 0) {
            ...
            if (seq_num != 0 && !(seq_num == 1 && ack_num == 1 && copy->GetSize () == 0)){ // not deal with 3-way handshaking & control msg
                if (copy->GetSize () == 0) { // if packet is ACK
                    if (AggregateAck.GetValue() < ack_num){
                        AggregateAck = SequenceNumber32(ack_num);
                    }
                    for (auto WorkerNeedAcked : DupAck(ack_num)){
                    ...
                    if (it != CountMap.end()){ // seq in CountMap
                        ...
                        if (UpdateMap(pkt, Worker)){ 
                            ...
                            uint8_t aggregate_payload[4]; 
                            aggregate_payload[0] = 0;
                            aggregate_payload[1] = 0;
                            aggregate_payload[2] = (pkt.payload >> 8) & 0xFF;
                            aggregate_payload[3] = pkt.payload & 0xFF;
                            // create new pkt
                            Ptr<Packet> newPacket = Create<Packet>(aggregate_payload, 4);
