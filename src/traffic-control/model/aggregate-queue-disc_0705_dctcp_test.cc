...
std::vector<int> eceNum(WorkerNum + 1);
int cwrNum = 0;
//////////////////////////////////////////////////////
...
void Chase(aggregate_pkt pkt, int Worker){
    if (Worker_chase[Worker].chaseFlag && pkt.wid >= Worker_chase[Worker].chaseWid){
        Worker_chase[Worker].chaseGap = (Worker_chase[Worker].chaseGap * 2 + ((*max_wid)[0] - Worker_chase[Worker].lastBiggest)) / 3;
    }
...
std::vector<std::pair<int, long long>> Retransmission(int ackWid, std::vector<std::pair<int, int>> intSackList){
    // std::cout << "Retransmission!\n";
    std::vector<std::pair<int, long long>> pktList;
    for (auto pair : intSackList){
        while(ackWid < pair.first){
            ...
            if (seq_num != 0 && !(seq_num == 1 && ack_num == 1 && copy->GetSize () == 0) && (ttl != 49) && (ttl != 50)){ // not deal with 3-way handshaking & control msg
                ...
                if (src_info.first == psAddr) { // if packet is ACK
                    ...
                    if (AggregateAck < ack_num){
                        ...
                        if (dupAckCount == 3 || (dupAckCount % 12) == 0){
                            ...
                            if (tcpHeader.GetOption(ns3::TcpOption::SACK)){
                                Ptr<const TcpOptionSack> sack = DynamicCast<const TcpOptionSack>(tcpHeader.GetOption(ns3::TcpOption::SACK));
                                TcpOptionSack::SackList sackList = sack->GetSackList();
                                // std::cout << "SackList : \n";
                                for (auto pairs : sackList){
                                    intSackList.push_back(std::pair<int, int>((pairs.first.GetValue() - 1) / payloadSize, (pairs.second.GetValue() - 1) / payloadSize));
                                    // std::cout << pairs.first.GetValue() << " " << pairs.second.GetValue() << "\n";
                                }
                            }
                            pktList = Retransmission(ackWid, intSackList);
                        }
                        else{ // ignore
                            return false;
                        }
                    ...
                    for (auto ackData : ackList){
                        ...
                        // if (eceNum[ackData.first] >= 10){
                        //     newTCPheader.SetFlags(newTCPheader.GetFlags() | TcpHeader::ECE); // set ECN
                        //     eceNum[ackData.first] -= 10;
                        // }
                        if (nQueued > maxnQueued * 0.95){
                            newTCPheader.SetFlags(newTCPheader.GetFlags() | TcpHeader::ECE); // set ECN
                        }
                    ...
                    for (auto pkt : pktList){
                        ...
                        // if (cwrNum >= WorkerNum){
                        //     out_tcpHeader.SetFlags(out_tcpHeader.GetFlags() | TcpHeader::CWR); // set ECN
                        //     cwrNum -= WorkerNum;
                        // }
                        ...
                        uint32_t nQueued = pktQueue->GetCurrentSize().GetValue();
                        uint32_t maxnQueued = pktQueue->GetMaxSize().GetValue();
                        // if (nQueued > 0.95 * maxnQueued){
                        //     newheader.SetEcn(Ipv4Header::EcnType::ECN_CE);
                        // }
                    ...
                    pktQueue = GetInternalQueue(0);
                    // std::cout << "PKT pktnum = " << pktnum << "\n";
                    ...
                    // if (getFlag && pkt.seq.GetValue() == 8001){
                    //     ackList.push_back(std::pair<int, int>(Worker, pkt.seq.GetValue()));
                    // }
                    for (auto ackData : ackList){
                        std::cout << "=> sent ack " << ackData.second + payloadSize << " to worker : " << ackData.first << "\n";
