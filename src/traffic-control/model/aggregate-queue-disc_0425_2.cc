        ...
        if (pkt.wid < (*min_wid)[0]){
            ...
            if (pkt.wid >= Worker_chase[Worker].chaseWid){
                if (!Worker_chase[Worker].chaseFlag){
                    ...
                    int gap = (*max_wid)[0] - Worker_chase[Worker].lastBiggest;
                    // double rate = gap / 
                    Worker_chase[Worker].chaseWid = (*max_wid)[0] + gap;
                    Worker_chase[Worker].lastBiggest = (*max_wid)[0];
                    std::cout << "chase " << Worker_chase[Worker].chaseWid << "!\n";
                }
            }
            ...
            std::cout << "wid " << pkt.wid << " in max and min\n";
            ...
            for (auto it = min_wid; ; ){
                ...
                if (((*it)[0] == pkt.wid)){
                    std::cout << "wid : " << pkt.wid << " = (*it)[0] : " << (*it)[0] << " (*it)[0] == pkt.wid => " << ((*it)[0] == pkt.wid) << "\n";
                    if ((*it)[2] != WorkerNum){
                        if ((*it)[Worker] == 0){
