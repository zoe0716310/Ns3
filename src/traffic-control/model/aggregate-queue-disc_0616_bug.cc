...
std::vector<std::pair<int, int>> globalAckList;
std::vector<std::pair<int, int>> globalPktList;
SequenceNumber32 AggregateSeq;
int AggregateAck = 0;
int dupAckCount = 0;
uint8_t AggregateCount;
int retransmission_times = 0;
...
std::pair<std::vector<std::pair<int, int>>, std::vector<std::pair<int, int>>> UpdateTable(aggregate_pkt pkt, u_int32_t Worker){
    std::cout << "*****************************\n";
    ...
    std::vector<std::pair<int, int>> pktList;

    // put pkt into table
    if (!AggregateTable.empty()){ // if table not empty
        // std::cout << "table not empty!" << "\n";
        bool isDiactly = UpdateDirectly(pkt.wid, pkt.payload);
        // if (Worker_chase[Worker].chaseFlag && pkt.wid >= Worker_chase[Worker].chaseWid){
        //     Worker_chase[Worker].chaseGap = (*max_wid)[0] - Worker_chase[Worker].lastBiggest;
        // }
        if ((*max_wid)[0] - pkt.wid > QueueLength / 4){
            Chase(pkt, Worker);
        }
        // Chase(pkt, Worker);
        if (pkt.wid < (*min_wid)[0]){
            ...
            for (int i = 1; i <= gap; i++){
                ...
                if (i == gap){ // new max_wid
                    ...
                    if ((*now_wid)[WorkerNum + 2] == WorkerNum && max_wid == now_wid){
                        max_wid = temp_wid;
                        now_wid = temp_wid;
                    }
                    else{
...
std::vector<std::pair<int, int>> Retransmission(int ackWid){
    // std::cout << "Retransmission!\n";
    retransmission_times++;
    std::cout << "retransmission times : " << retransmission_times << "\n";
    std::vector<std::pair<int, int>> pktList;
        ...
        if (copy->PeekHeader(tcpHeader) != 0) {
            ...
            if (seq_num != 0 && !(seq_num == 1 && ack_num == 1 && copy->GetSize () == 0) && (ttl != 48)){ // not deal with 3-way handshaking & control msg
                ...
                if (src_info.first == "10.0.11.2") { // if packet is ACK
                    ...
                    std::cout << "\n";
                    if (!globalAckList.empty()){
                        std::cout << "globalAckList = ";
                        for (auto ack : globalAckList){
                            std::cout << ack.second << " ";
                        }
                        std::cout << "\n";
                        if (!ackList.empty()){
                            ackList.insert(ackList.end(), globalAckList.begin(), globalAckList.end());
                        }
                        else{
                            ackList = globalAckList;
                        }
                        std::cout << "new ackList = ";
                        for (auto ack : ackList){
                            std::cout << ack.second << " ";
                        }
                        std::cout << "\n";
                    ...
                    for (auto ackData : ackList){
                        ...
                        if(tcpHeader.GetOption(ns3::TcpOption::SACK)){
                            ...
                            newTCPheader.AppendOption(tcpHeader.GetOption(ns3::TcpOption::TS));
                        }
                        else{
                            newTCPheader = tcpHeader;
                        }
                    ...
                    if (!pktList.empty()){
                        if (!globalPktList.empty()){
                            globalPktList.insert(globalPktList.end(), pktList.begin(), pktList.end());
                        }
                        else{
                            globalPktList = pktList;
                        }
                    }
                    ...
                    std::pair<std::vector<std::pair<int, int>>, std::vector<std::pair<int, int>>> AggregateParam = UpdateTable(pkt, Worker);
                    std::vector<std::pair<int, int>> ackList = AggregateParam.first;
                    std::vector<std::pair<int, int>> pktList = AggregateParam.second;
                    if (!ackList.empty()){
                        if (!globalAckList.empty()){
                            globalAckList.insert(globalAckList.end(), ackList.begin(), ackList.end());
                        }
                        else{
                            globalAckList = ackList;
                        }
                    }
                    ...
                    // }
                    std::cout << "pktList = ";
                    for (auto ack : pktList){
                        std::cout << ack.first << " ";
                    }
                    std::cout << "\n";
                    std::cout << "globalPktList = ";
                    for (auto ack : globalPktList){
                        std::cout << ack.first << " ";
                    }
                    std::cout << "\n";
                    int sz = 0;
                    if (globalPktList.size() > 20){
                        std::vector<std::pair<int, int>> tempList = pktList;
                        pktList.clear();
                        pktList.insert(pktList.end(), globalPktList.begin(), globalPktList.begin() + 19);
                        globalPktList.erase(globalPktList.begin(), globalPktList.begin() + 19);
                        globalPktList.insert(globalPktList.end(), tempList.begin(), tempList.end());
                    }
                    else if(globalPktList.size() + pktList.size() > 20){
                        sz = 20 - globalPktList.size();
                        if (sz == 0){
                            std::vector<std::pair<int, int>> tempList = pktList;
                            pktList = globalPktList;
                            globalPktList.clear();
                            globalPktList.insert(globalPktList.end(), tempList.begin(), tempList.end());
                        }
                        else{
                            std::vector<std::pair<int, int>> tempList = pktList;
                            pktList = globalPktList;
                            pktList.insert(pktList.end(), tempList.begin(), tempList.begin() + sz - 1);
                            tempList.erase(tempList.begin(), tempList.begin() + sz - 1);
                            globalPktList = tempList;
                        }
                    }
                    else{
                        std::vector<std::pair<int, int>> tempList = pktList;
                        pktList = globalPktList;
                        pktList.insert(pktList.end(), tempList.begin(), tempList.end());
                        globalPktList.clear();
                    }
                    std::cout << "new pktList = ";
                    for (auto ack : pktList){
                        std::cout << ack.first << " ";
                    }
                    std::cout << "\n";
