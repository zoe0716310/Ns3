...
AggregateQueueDisc::Enqueue(Ptr<QueueDiscItem> item)
{
    ...
    if (Mycount1 % 1 == 0){
        if (copy->PeekHeader(tcpHeader) != 0) {
            ...
            bool isBackground = (dst_info.second == 12345) || (src_info.second == 12345);

            if (!isBackground && seq_num != 0 && !(seq_num == 1 && ack_num == 1 && copy->GetSize () == 0)){ // not deal with 3-way handshaking & control msg
