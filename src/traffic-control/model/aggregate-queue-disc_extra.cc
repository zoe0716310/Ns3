...
void AggregateQueueDisc::SetAnotherDisc(Ptr<ns3::QueueDisc> disc){
    anotherDisc = disc;
}
...
std::vector<std::pair<int, long long>> Retransmission(int ackWid, std::vector<std::pair<int, int>> intSackList){
    ...
    if (!pktList.empty()){
        if (!isAckWid){
            pktList.insert(pktList.begin(), {ackWid, 0});
            hasSentVector.insert(hasSentVector.begin(), {hasSent[0], hasSent[1]});
        }
        return pktList;
    }
    if (tempWid < (*min_wid)[0]){ // in directly_map
            ...
            if (seq_num != 0 && !(seq_num == 1 && ack_num == 1 && copy->GetSize () == 0) && (ttl != 49) && (ttl != 50)){ // not deal with 3-way handshaking & control msg
                ...
                if (src_info.first == psAddr) { // if packet is ACK
                    ...
                    for (auto ackData : ackList){
                        ...
                        receivedPackets[ackData.first - 1].insert(ackData.second);
                        // Update lastAck if the packet fills a gap
                        while (receivedPackets[ackData.first - 1].find(lastAck[ackData.first - 1]) != receivedPackets[ackData.first - 1].end()) {
                            lastAck[ackData.first - 1] += payloadSize;
                        }
...
AggregateQueueDisc::DoEnqueue(Ptr<QueueDiscItem> item)
{
    ...
    if (GetCurrentSize() + item > GetMaxSize())
    {
        ...
        std::cout << "Queue full -- dropping pkt\n";
        return false;
    }
