...
uint8_t AggregateCount[2];
std::vector<int> directlySent;
int payloadSize = 200;
int totalSend = 0;
std::vector<int> workerHasSent(WorkerNum);
//////////////////////////////////////////////////////
...
std::pair<bool, long long> NeedAggregateOrNot(){
...
void ClearTable(){
    int min_ele = *std::min_element(workerHasSent.begin(), workerHasSent.end());
    if(AggregateTable.empty()){
        return;
    }
    for (int i = 0; i < AggregateTable[0].size(); i++){
        if (AggregateTable[0][i] < min_ele){
            for (int k = 0; k <= WorkerNum; k++){ // delete corresponding seq on table
                AggregateTable[k].erase(AggregateTable[k].begin() + i);
            }
            i--;
    ...
    directlySent.erase(std::remove_if(directlySent.begin(), directlySent.end(), [min_ele](int x) {
        return x < min_ele;
    }), directlySent.end());
}
    ...
    if (Mycount1 % 1 == 0){
        if (copy->PeekHeader(udpHeader) != 0) {
            ...
            if (1){ // not deal with 3-way handshaking & control msg
                ...
                workerHasSent[Worker - 1] = pkt.payload;
                ...
                if (nQueued > 0.95 * maxnQueued){
                    // ecnBit = true;
                    setBit(AggregateCount, 11); // set ECN
                }
                // ClearTable();
                ...
                if (AggregateParam.first){
                    ...
                    aggregate_payload[1] = AggregateCount[1];
