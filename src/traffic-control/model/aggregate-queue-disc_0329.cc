...
////////////////////////////////////////////////////// global var
uint32_t WorkerNum = 2; // Workers per aggregate
uint32_t QueueLength = 20; // max packet number in aggregate table per worker
uint32_t AggregateSsh = 2; // threshold of aggregate
...
std::map<SequenceNumber32, std::bitset<2>> CountMap; // map from seq to count
...
std::pair<bool, long long> NeedAggregateOrNot(bool directlySent = false){
    ...
    if (directlySent){
        ...
        return std::pair<bool, long long> (true, AggregatePayload / (4 - AggregateCount));
    }
    for (int i = 0; i < AggregateTable[0].size(); i++){
        ...
        // std::cout << "WorkerArrive : " << WorkerArrive << "\n";
        if (WorkerArrive >= AggregateSsh){ // need aggregate
            ...
            return std::pair<bool, long long> (true, AggregatePayload / WorkerArrive);
        }
    }
...
bool UpdateMap(aggregate_pkt pkt, u_int32_t Worker){
    ...
    if (CountMap[seq][Worker - 1] == 1){
        ...
        for(int i = 0; i < AggregateTable[0].size(); i++){
            if (AggregateTable[0][i] == pkt.payload){
                for (int j = 0; j < AggregateTable.size(); j++){
                    AggregateTable[j].erase(AggregateTable[j].begin() + i);
                }
            }
        }
    ...
    if (Mycount1 % 1 == 0){
        if (copy->PeekHeader(udpHeader) != 0) {
            ...
            if (1){ // not deal with 3-way handshaking & control msg
                ...
                // std::cout << "Worker " << Worker << " sent " << pkt.payload << "\n";
                uint32_t nQueued = GetInternalQueue(0)->GetCurrentSize().GetValue();
                uint32_t maxnQueued = GetInternalQueue(0)->GetMaxSize().GetValue();
                short ecnBit;
                ...
                auto it = CountMap.find(SequenceNumber32(pkt.payload));
                if (it != CountMap.end()){ // seq in CountMap
                    // std::cout << "CountMap has seq!" << "\n";
                    if (UpdateMap(pkt, Worker)){ 
                        std::pair<Ipv4Address, uint16_t> LeaderTuple = Workers_Num[1];
                        // change int to uint8_t
                        uint8_t aggregate_payload[200]; 
                        aggregate_payload[0] = Worker;
                        aggregate_payload[1] = ecnBit;
                        aggregate_payload[2] = (pkt.payload >> 8) & 0xFF;
                        aggregate_payload[3] = pkt.payload & 0xFF;
                        // create new pkt
                        ...
                        out_udpHeader.ForcePayloadSize(8 + 200);
                    ...
                    if (!UpdateTable(pkt, Worker)){
                        AggregateParam.first = true;
                        AggregateParam.second = pkt.payload;
                        AggregateCount = Worker;
                    }
                    else{
                        AggregateParam = NeedAggregateOrNot();
                        AggregateCount = 3; // async
                    }
                    // for (int i = 0; i < AggregateTable.size(); i++){
                    //     for (int j = 0; j < AggregateTable[0].size(); j++){
                    //         std::cout << AggregateTable[i][j] << " ";
                    //     }
                    //     std::cout << "\n";
                    // }
                    if (AggregateParam.first){
                        ...
                        // change int to uint8_t
                        uint8_t aggregate_payload[200]; 
                        aggregate_payload[0] = AggregateCount;
                        aggregate_payload[1] = ecnBit;
