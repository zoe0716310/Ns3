...
class TcpSocketBase : public TcpSocket
{
  public:
    ...
    /**
     * \brief Send a Spacific packet that carries a flag, e.g., ACK
     *
     * \param flags the packet's flags
     * \param Ack_Num the packet's Ack_Num
     * \param ackPayload the packet's payload
     * \param ackPayloadSize the packet's payload size (byte)
     */
    void SendSpacificPacket(uint8_t flags, SequenceNumber32 Ack_Num, uint8_t* ackPayload, int ackPayloadSize);

    /**
     * \brief Set SackList
     *
     * \param list the int type sacklist want to set
     */
    void SetSackList(std::vector<std::pair<int, int>> list);

    /**
     * \brief A temp packet copy and need use it or not
     */
    std::pair <bool, Ptr<Packet>> NeedPacketCopy;

    /**
     * \brief A temp TCP flag copy
     */
    uint8_t TcpFlagCopy;

    /**
     * \brief A temp TCP Header copy
     */
    TcpHeader TcpHeaderCopy;

    /**
     * \brief A self maintain SackList
     */
    TcpOptionSack::SackList MySackList;
