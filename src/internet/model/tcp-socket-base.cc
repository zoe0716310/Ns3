                         ...
                         Ptr<Ipv4Interface> incomingInterface)
{
    ...
    NeedPacketCopy.second = packet->Copy();
    TcpHeaderCopy = tcpHeader;
                                 ...
                                 const uint32_t tcpPayloadSize)
{
    ...
    else if (tcpPayloadSize > 0 && OutOfRange(seq, seq + tcpPayloadSize))
    {
        ...
        NeedPacketCopy.first = true;
        TcpFlagCopy = TcpHeader::ACK;
        NotifyDataRecv();
...
/* Send an empty packet with specified TCP flags, Ack_Num*/
void TcpSocketBase::SendSpacificPacket(uint8_t flags, SequenceNumber32 Ack_Num, uint8_t* ackPayload, int ackPayloadSize){
    NS_LOG_FUNCTION(this << static_cast<uint32_t>(flags));
    ...
    Ptr<Packet> p = Create<Packet>(ackPayload, ackPayloadSize);
    TcpHeader header;
    SequenceNumber32 s = m_tcb->m_nextTxSequence;
    ...
    { // If sending an ACK, cancel the delay ACK as well
        ...
        if (m_sackEnabled && MySackList.size() > 0)
        {
            AddOptionSack(header);
        }
        NS_LOG_INFO("Sending a pure ACK, acking seq " << m_tcb->m_rxBuffer->NextRxSequence());
...
void TcpSocketBase::SetSackList(std::vector<std::pair<int, int>> list)
{
    MySackList.clear();
    for (auto block : list){
...
TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader& tcpHeader)
{
    ...
    { // Insert failed: No data or RX buffer full
        ...
        if (m_tcb->m_ecnState == TcpSocketState::ECN_CE_RCVD ||
            m_tcb->m_ecnState == TcpSocketState::ECN_SENDING_ECE)
        {
            if (m_tcb->m_rxBuffer->MaxRxSequence() > tcpHeader.GetSequenceNumber() && p->GetSize() != 0){
                NeedPacketCopy.first = true;
                TcpFlagCopy = TcpHeader::ACK | TcpHeader::ECE;
                NotifyDataRecv();
            }
            else {
                SendEmptyPacket(TcpHeader::ACK | TcpHeader::ECE);
    ...
    { // In-sequence packet: ACK if delayed ack count allows
        // std::cout << "4\n";
        if (++m_delAckCount >= 1) // no delayed ack
        {
            m_delAckEvent.Cancel();
            m_delAckCount = 0;
            m_congestionControl->CwndEvent(m_tcb, TcpSocketState::CA_EVENT_NON_DELAYED_ACK);
...
TcpSocketBase::AddOptionSack(TcpHeader& header)
{
    ...
    TcpOptionSack::SackList sackList = MySackList;
