...
void ReceiveTCPAck (Ptr<Socket> socket)
{
    ...
    int max_wid = 0;
    for (int i = 0; i < 2; i++) {
        now_wid = (now_wid << 8) | buffer[i];
    }
    for (int i = 2; i < 4; i++) {
        max_wid = (max_wid << 8) | buffer[i];
    }
    if (socket->GetNode()->GetId() == 8){
        std::cout << "now_wid : " << now_wid << "\n";
        std::cout << "max_wid : " << max_wid << "\n";
    }

    app[socket->GetNode()->GetId()]->SetSwitchRate(max_wid, now_wid);
    
    // if (max_wid - app[socket->GetNode()->GetId()]->GetpacketsSent() > 0){
    //     std::cout << "gap with max_wid : " << max_wid - app[socket->GetNode()->GetId()]->GetpacketsSent() << "\n";
    // }
...
void SendTCPData(Ptr<Socket> socket, uint32_t size){
    app[socket->GetNode()->GetId()]->SetWorkerRate();
...
main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < WorkerNum; i++){
        ...
        app[i]->Setup(ns3TcpSockets[i], sinkAddress, payloadSize + 54, totalPkt, DataRate(std::to_string(workerBw[i]) + "Mbps"));
