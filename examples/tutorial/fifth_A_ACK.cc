...
void ReceiveTCPPacket (Ptr<Socket> socket)
{
            ...
            if (unDoneMap[i].second.count() >= AggregateSsh){
                ...
                for (int j = 0; j < unDoneMap[i].second.size(); j++){
                    if (unDoneMap[i].second[j] == 1){
                        ...
                        if (smallestAck == 0 || lastAck[j] < smallestAck){
                            smallestAck = lastAck[j];
                            smallestAckNum = j;
                        }
                    }
                }
                ...
                tcpSocketBase->SetSackList(generateSackList(receivedPackets[smallestAckNum], lastAck[smallestAckNum]));
                //std::cout << "seq : " << currentSeq << " ack: " << lastAck[j] << "buf[0] : " << int(buf[0]) << "buf[1] : " << int(buf[1]) << "\n";
                tcpSocketBase->SendSpacificPacket(tcpSocketBase->TcpFlagCopy, SequenceNumber32(lastAck[smallestAckNum]), buf, 2);
    ...
    if (workersHasSent.size() >= AggregateSsh){
        ...
        for (auto worker : workersHasSent){
            ...
            temp_vec.push_back(worker);
            if (smallestAck == 0 || lastAck[worker - 1] < smallestAck){
                smallestAck = lastAck[worker - 1];
                smallestAckNum = worker - 1;
            }
        }
...
void ReceiveUDPPacket (Ptr<Socket> socket){
    ...
    int cnt = Count;
    cnt = WorkerNum - cnt;
    std::cout << "ReceiveUDPPacket seq : " << payload << "; count : " << cnt << "\n";
    UdpMap[payload].first -= 1;
    if (UdpMap[payload].second < cnt){
        if (UdpMap[payload].second != 0){
            successAggregate++;
        }
