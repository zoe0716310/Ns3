...
int pktSent = 60000;
std::vector<std::bitset<10>> workerHasSentMap(pktSent);
...
TraceThroughput(Ptr<FlowMonitor> monitor)
{
    ...
    Simulator::Schedule(Seconds(0.05), &TraceThroughput, monitor);
}

void TraceCwnd(){
    for(int i = 0; i < WorkerNum; i++){
        std::ofstream thr1(dir + "/cwnd" + std::to_string(i) + ".dat", std::ios::out | std::ios::app);
            thr1 << Now().GetSeconds() << " "
                << app[i]->GetCwnd()
                << std::endl;
    }
    ...
    for (auto worker : workerHasSent){
        if (workerHasSentMap[payload][worker] == 0){
            ...
            if (workerHasSentMap[payload].all()){
                successAggregate++;
                for(int i = 0; i < WorkerNum; i++){
                    Simulator::Schedule(MilliSeconds(0.6), &SendAck, payload, ecn, i);
                }
                // std::cout << "successAggregate\n";
            }
            totalPkt++;
        }
    }
...
main(int argc, char* argv[])
{
    ...
    normalVariable->SetAttribute("Mean", DoubleValue(mean));
    normalVariable->SetAttribute("Variance", DoubleValue(standard_deviation * standard_deviation));
    ...
    for (int i = 0; i < WorkerNum + 1; i++){
        ...
        if (i == WorkerNum){
            ...
            double tempMbps = normalVariable->GetValue();
    ...
    Simulator::Schedule(Seconds(0 + 0.000001), &TraceCwnd);
