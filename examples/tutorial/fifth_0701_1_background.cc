...
{
    *stream->GetStream() << Simulator::Now().GetSeconds() << " " << newval / payloadSize << std::endl;
}
...
void ReceiveTCPPacket (Ptr<Socket> socket)
{
    ...
    // std::cout << "\n";
    // for (auto seq : unDoneMap){
    //     int cnt = seq.second.to_ulong();
    //     std::cout << cnt << "    ";
    // }
    // std::cout << "\n";
    uint8_t * buf = new uint8_t[2];
    uint32_t i = 0;
    if (currentSeq.GetValue() == (payloadSize * (totalPkt - 1) + 1)){
    ...
    for (auto seq : unDoneMap){
        if (seq.first == currentSeq){
            ...
            //     std::cout << "In unDoneMap worker : " << worker << "\n";
            // }
            ...
            if (unDoneMap[i].second.count() >= AggregateSsh){
                ...
                //std::cout << "seq : " << currentSeq << " ack: " << lastAck[j] << "buf[0] : " << int(buf[0]) << "buf[1] : " << int(buf[1]) << "\n";
                tcpSocketBase->SendSpacificPacket(tcpSocketBase->TcpFlagCopy, SequenceNumber32(lastAck[0]), buf, 0);
                if (unDoneMap[i].second.count() == WorkerNum){
    ...
    if (workersHasSent.size() >= AggregateSsh){
        ...
        tcpSocketBase->SetSackList(generateSackList(receivedPackets[0], lastAck[0]));
        // std::cout << "seq : " << currentSeq << " ack: " << lastAck[worker - 1] << "\n";
        tcpSocketBase->SendSpacificPacket(tcpSocketBase->TcpFlagCopy, SequenceNumber32(lastAck[0]), buf, 0);
    }
    else{
        // for (auto worker : workersHasSent){
        //     std::cout << "Build unDoneMap : " << currentSeq << "; worker : " << worker << "\n";
        // }
...
void ReceiveBackgroundPacket (Ptr<Socket> socket)
{
    Ptr<TcpSocketBase> tcpSocketBase = DynamicCast<TcpSocketBase>(socket);
    Ptr<Packet> packet;
    TcpHeader tcpHeader;
    ...
    SequenceNumber32 currentSeq = tcpHeader.GetSequenceNumber();
    uint8_t * buf = new uint8_t[2];
    // std::cout << "currentSeq.GetValue() : " << currentSeq.GetValue() <<"\n";

    receivedPackets[WorkerNum].insert(currentSeq.GetValue());
    // Update lastAck if the packet fills a gap
    while (receivedPackets[WorkerNum].find(lastAck[WorkerNum]) != receivedPackets[WorkerNum].end()) {
        lastAck[WorkerNum] += payloadSize;
    }
    tcpSocketBase->SetSackList(generateSackList(receivedPackets[WorkerNum], lastAck[WorkerNum]));
    tcpSocketBase->SendSpacificPacket(tcpSocketBase->TcpFlagCopy, SequenceNumber32(lastAck[WorkerNum]), buf, 0);
}
...
void SetCallback_RecvBackground (Ptr<Socket> socket, const Address& addr){
    socket->SetRecvCallback (MakeCallback (&ReceiveBackgroundPacket));
}
...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("bigN", "Big N", bigN);
    cmd.AddValue("alpha", "Alpha", alpha);
    cmd.AddValue("seed", "Random Seed", seed);
    cmd.AddValue("hasBackground", "Has background flow", hasBackground);
    ...
    zipf->SetAttribute ("N", IntegerValue (bigN));
    zipf->SetAttribute ("Alpha", DoubleValue (alpha));
    ...
    PointToPointHelper backgroundLink;
    backgroundLink.SetDeviceAttribute("DataRate", StringValue(std::to_string(10) + "Mbps"));
    backgroundLink.SetChannelAttribute("Delay", StringValue("0.2ms"));
    ...
    bottleneckLink.SetChannelAttribute("Delay", StringValue("0.2ms"));
    ...
    NetDeviceContainer backgroundEdges;
    backgroundEdges = backgroundLink.Install(background.Get(0), routers.Get(0));
    ...
    Ipv4InterfaceContainer ib1 = ipv4.Assign(backgroundEdges);
    ...
    uint16_t background_port = 12345;

    //////////////////////////////////////////////////// TCP
    Address sinkAddress(InetSocketAddress(ir1.GetAddress(1), first_port));
    Address backgroundAddress(InetSocketAddress(ir1.GetAddress(1), background_port));
    ...
    Ptr<Socket> backgroundreceiverSocket = Socket::CreateSocket (receiver.Get (0), TcpSocketFactory::GetTypeId ());
    backgroundreceiverSocket->Bind (InetSocketAddress (Ipv4Address::GetAny (), background_port));
    backgroundreceiverSocket->Listen ();
    backgroundreceiverSocket->SetAcceptCallback(&BoolCallback, &SetCallback_RecvBackground);
    ...
    for (int i = 0; i < WorkerNum; i++){
        ...
        apps[i]->Setup(ns3TcpSockets[i], sinkAddress, payloadSize + 54, totalPkt, DataRate(std::to_string(workerBw[i]) + "Mbps"), 0);
        sender.Get(i)->AddApplication(apps[i]);
        apps[i]->SetStartTime(Seconds(0.));
        Simulator::Schedule(Seconds(0.) + MilliSeconds(1), &TraceCwnd, i, 0);
        apps[i]->SetStopTime(stopTime);
    }

    Ptr<Socket> ns3BackgroundTcpSockets;
    Ptr<TutorialApp> backgroundapp;
    if (hasBackground){
        ns3BackgroundTcpSockets = Socket::CreateSocket(background.Get(0), TcpSocketFactory::GetTypeId());
        backgroundapp = CreateObject<TutorialApp>();
        backgroundapp->Setup(ns3BackgroundTcpSockets, backgroundAddress, payloadSize + 54, totalPkt, DataRate("10Mbps"), 0);
        background.Get(0)->AddApplication(backgroundapp);
        backgroundapp->SetStartTime(Seconds(0.4));
        backgroundapp->SetStopTime(Seconds(0.6));
    }
    // /////////////////////////////////////////////////////////////////
