...
int totalPkt = 20000;
std::vector<std::bitset<10>> workerHasSentMap(totalPkt);
int receivedUdp = 0;
...
void ReceiveTCPPacket (Ptr<Socket> socket)
{
    ...
    int wid = (currentSeq.GetValue() - 1) / payloadSize;
    for (auto worker : workersHasSent){
        workerHasSentMap[wid][worker - 1] = 1;
    }
    if (wid == totalPkt - 1){
        for (int i = 0; i < workerHasSentMap.size(); i++){
            std::cout << i << " : ";
            for(int j = 0; j < workerHasSentMap[i].size(); j++){
                std::cout << workerHasSentMap[i][j];
            }
            std::cout << std::endl;
        }
        std::cout << "Total Send : " << total_send << "\n";
...
void ReceiveTCPAck (Ptr<Socket> socket)
{
    ...
    if (tcpSocketBase->NeedPacketCopy.first){
        // std::cout << "tcpSocketBase ack set packet =\n";
        packet = tcpSocketBase->NeedPacketCopy.second;
        tcpSocketBase->NeedPacketCopy.second->RemoveHeader(tcpHeader);
        // std::cout << "tcpSocketBase ack set packet = End\n";
    }
    else{
        // std::cout << "socket ack set packet =\n";
        packet = socket->RecvFrom (from);
        tcpHeader = tcpSocketBase->TcpHeaderCopy;
    }
...
void SendTCPData(Ptr<Socket> socket, uint32_t size){
    app[socket->GetNode()->GetId()]->PopTxbuffer();
...
main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < WorkerNum; i++){
        ...
        ns3TcpSockets[i]->SetDataSentCallback(MakeCallback (&SendTCPData));
        app[i] = CreateObject<TutorialApp>();
        app[i]->Setup(ns3TcpSockets[i], sinkAddress, payloadSize + 54, totalPkt, DataRate(std::to_string(workerBw[i]) + "Mbps"), 0);
