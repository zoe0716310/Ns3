...
int AggregateSsh = 2; // threshold of aggregate
...
std::map<uint32_t, uint32_t> UdpMap;
...
void ReceiveUDPPacket (Ptr<Socket> socket){
    ...
    int cnt = Count;
    // std::cout << "Receive count " << cnt << " seq : " << payload << "\n";
    cnt = 3 - cnt;
    if (cnt == 0){
        app[0]->Ack(payload);
        app[1]->Ack(payload);
        successAggregate++;
        // std::cout << "successAggregate : " << payload << "\n";
    }
    else{
        if (UdpMap.find(payload) != UdpMap.end()){ //exist
            if (UdpMap[payload] + cnt == 3){
                app[0]->Ack(payload);
                app[1]->Ack(payload);
                successAggregate++;
                // std::cout << "successAggregate : " << payload << "\n";
    ...
    std::cout << Simulator::Now().GetSeconds() << " " << successAggregate << "\n";
}
...
main(int argc, char* argv[])
{
    ...
    app[0]->SetStartTime(Seconds(0.1));
    app[0]->SetStopTime(stopTime);

    Ptr<Socket> ns3UdpSocket2 = Socket::CreateSocket(sender.Get(1), UdpSocketFactory::GetTypeId());
    app[1]->Setup(ns3UdpSocket2, sinkAddress, packetSize, maxPacketCount, DataRate(std::to_string(SendingRate_max) + "Mbps"));
    sender.Get(1)->AddApplication(app[1]);
    app[1]->SetStartTime(Seconds(0.1));
