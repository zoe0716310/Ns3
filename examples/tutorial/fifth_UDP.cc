...
void ReceiveTCPPacket (Ptr<Socket> socket)
{
    ...
    if (workersHasSent[0] <= (WorkerNum - AggregateSsh)){ // Fix
        int worker = workersHasSent[0];
        ...
        clearBuffer(buf);
        setBit(buf, worker);
        tcpSocketBase->SetSackList(generateSackList(receivedPackets[worker - 1], lastAck[worker - 1]));
        //std::cout << "seq : " << currentSeq << " ack: " << lastAck[worker - 1] << "\n";
        tcpSocketBase->SendSpacificPacket(tcpSocketBase->TcpFlagCopy, SequenceNumber32(lastAck[worker - 1]), buf, 2);
        return;
    }
    ...
    if (workersHasSent.size() >= AggregateSsh){
        ...
        // if (workersHasSent.size() != WorkerNum){ //Fix
        //     // for (auto worker : workersHasSent){
...
void ReceiveUDPPacket (Ptr<Socket> socket){
    ...
    std::vector<int> workerHasSent;
    for (int i = 0; i < 4; i++){
        if ((buffer[0] & (1 << i)) != 0){
            workerHasSent.push_back(i);
        }
    }
    // std::cout << " worker : " << workerHasSent.size() << "\n";
...
main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < WorkerNum + 1; i++){
        ...
        edgeLinks[i].SetChannelAttribute("Delay", StringValue("5ms"));
    }
    ...
    app[0]->Setup(ns3UdpSocket1, sinkAddress, packetSize, maxPacketCount, DataRate(std::to_string(SendingRate[0]) + "Mbps"), 0);
    ...
    app[1]->Setup(ns3UdpSocket2, sinkAddress, packetSize, maxPacketCount, DataRate(std::to_string(SendingRate[1]) + "Mbps"), 0);
    ...
    app[2]->Setup(ns3UdpSocket3, sinkAddress, packetSize, maxPacketCount, DataRate(std::to_string(SendingRate[2]) + "Mbps"), 0);
    ...
    app[3]->Setup(ns3UdpSocket4, sinkAddress, packetSize, maxPacketCount, DataRate(std::to_string(SendingRate[3]) + "Mbps"), 0);
    ...
    //     apps[i]->Setup(ns3TcpSockets[i], sinkAddress, payloadSize + 54, packetNum, DataRate(std::to_string(workerBw[i]) + "Mbps"));
