...
Time average_delay = Time(0);

void 
TutorialApp::Ack(int seq, bool ecn)
{
    auto it = std::find_if(m_unAckedList.begin(), m_unAckedList.end(), [seq](const std::pair<uint32_t, Time>& element) {
        return element.first == seq;
    });
    if (it != m_unAckedList.end()) {
        average_delay = average_delay + Now() - it->second;
        // std::cout << "average_delay : " << average_delay.GetSeconds() << "\n";
    }
    m_unAckedList.erase(
        std::remove_if(m_unAckedList.begin(), m_unAckedList.end(),
                       [seq](const std::pair<uint32_t, Time>& element) {
                           return element.first == seq;
                       }),
        m_unAckedList.end());
    m_txbuffer.erase(
        std::remove_if(m_txbuffer.begin(), m_txbuffer.end(),
                       [seq](const int& element) {
                           return element == seq;
                       }),
        m_txbuffer.end());
    if (m_unAckedList.empty()){
        std::cout << "retransmission times : " << m_retransmissionTimes << "\n";
        std::cout << "average_delay : " << average_delay.GetSeconds() << "\n";
    }
}
...
TutorialApp::SendPacket()
{
    ...
    if(m_packetsSent < m_nPackets){
        // std::cout << "worker " << m_socket->GetNode()->GetId() << " SendPacket push : " << m_packetsSent << "\n";
        m_txbuffer.push_back(m_packetsSent);
        m_packetsSent++;
    }

    if(!m_txbuffer.empty()){
        // std::cout << "worker " << m_socket->GetNode()->GetId() << " buffer : ";
        // for(auto num : m_txbuffer){
        //     std::cout << num << " ";
        // }
        // std::cout << "\n";
        payload[0] = 0;
        payload[1] = 0;
        payload[2] = (m_txbuffer[0] >> 8) & 0xFF;
        payload[3] = m_txbuffer[0] & 0xFF;
        Ptr<Packet> packet = Create<Packet>(payload, 200);
        std::pair<uint32_t, Time> packetTag(m_txbuffer[0], Now());
        m_unAckedList.push_back(packetTag);
        // std::cout << "worker " << m_socket->GetNode()->GetId() << "SendPacket : " << m_txbuffer[0] << "\n";
...
TutorialApp::Retransmission(int seq, bool isTimeout)
{
    ...
    Ptr<Packet> packet = Create<Packet>(payload, 200);
    // std::cout << "worker " << m_socket->GetNode()->GetId() << " Retransmission push : " << seq << "\n";
...
TutorialApp::SetTimeOut(){
    ...
    // }
    // std::cout << "\n";
    int temp = m_txbuffer[0];
    // m_txbuffer.erase(m_txbuffer.begin());
    ScheduleCheckAck(temp);
