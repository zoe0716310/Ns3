      ...
      for to in "${timeout[@]}"; do
        for s in "${seed[@]}"; do
          for ssh in "${AggregateSsh[@]}"; do
            # 生成目录路径
            dir="logs/0527/50p_0.2ms/Reliable/Timeout_${to}ms/UDP_50bf_${bw}Bw_${mean_rate}_standard_deviation${delta}/seed_${s}"
            ...
            # 生成命令
            command="./ns3 run fifth -- --mean=$mean_rate --standard_deviation=$delta  --timeout=$to  --outputPcapDir=logs/0527/50p_0.2ms/Reliable/Timeout_${to}ms/UDP_50bf_${bw}Bw_${mean_rate}_standard_deviation${delta}/seed_${s} --outgoingBW=$bw --seed=$s --AggregateSsh=$ssh > logs/0527/50p_0.2ms/Reliable/Timeout_${to}ms/UDP_50bf_${bw}Bw_${mean_rate}_standard_deviation${delta}/seed_${s}.txt &"
